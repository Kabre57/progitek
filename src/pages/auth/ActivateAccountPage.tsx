...
export const ActivateAccountPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // En mode démo, vérifier si le token existe dans les utilisateurs en attente
    const pendingUsers = JSON.parse(localStorage.getItem('demo_pending_users') || '{}');
    const userWithToken = Object.values(pendingUsers).find((user: any) => 
      user.registrationToken === token
    );
    
    setIsValidToken(!!userWithToken);
  }, [token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { user, token: authToken } = await authService.activateAccount(token, password);
      
      // Stocker le token et rediriger
      localStorage.setItem('token', authToken);
      
      toast.success(`Bienvenue ${user.prenom} ! Votre compte a été activé avec succès.`);
      navigate('/');
    } catch (error: any) {
        ...
        <div className="max-w-md w-full">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Le lien d'activation que vous avez utilisé est invalide ou a expiré. Veuillez demander un nouveau lien d'activation.
            </p>
      ...
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Définissez votre mot de passe pour terminer l'inscription
          </p>
        </div>
